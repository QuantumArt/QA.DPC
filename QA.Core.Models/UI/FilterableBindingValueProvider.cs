...
{
	/// <summary>
	/// позволяет фильтровать по синтаксису (EBNF):
	/// { field_name [ "[" [ ! ] { field_name } [ /...n ] "=" 'field_value' "]" [ ...n ] ] } [ /...n ]
	/// где [ ] для фильтрациии а ! - признак логической инверсии условия фильтрации
	/// </summary>
	public class FilterableBindingValueProvider : ReflectedBindingValueProvider
	{
		...
		{
			string targetPath = be.Expression;

			if(!(context is Article))
				throw new ArgumentException("context must be Article");

			return EvaluatePath(targetPath, (Article) context).Select(x => x.ModelObject).ToArray();
		}
        ...
        /// <returns>ArticleField или Article если в конце фильтр</returns>
						...
						.SelectMany(x => GetChildArticlesOrSelf(x.ModelObject)
                            .Where(y =>
								filterInfos
									.All(filterInfo =>
									{
										bool filterConditionMached = EvaluatePath(filterInfo.Path, y)
											.Any(
