DefinitionEditor = {
    ...
    init: function (saveToDb, saveDefinitionUrl, getSingleNodeUrl, getDefinitionLevelUrl, editUrl) {
        ...
        if (DefinitionEditor._saveToDb)
            ...
            pmrpc.call({
                ...
                onError: function (statusObj) { console.log("Error calling DefinitionEditorLoaded", statusObj); }
            });
        }
    },
    ...
    SaveXml: function () {
        ...
        var xml = $("#xmlViewerDiv").data("editor").getValue();
        ...
        if (DefinitionEditor._saveToDb) {
            ...
            $.ajax({
                ...
                type: "POST",
                data: { xml: xml },
                success: function () { DefinitionEditor.getToolbar().enable("#SaveButton"); },
                error: function (result) { $("#propsPane").html(result.responseText); },
                complete: function () { kendo.ui.progress($(document.body), false); }
            });
        } else {
            pmrpc.call({
                destination: parent,
    ...
    UpdateNode: function (path) {
        ...
        $.ajax({
            ...
            cache: false,
            type: "POST",
            data: { path: path, xml: $("#xmlViewerDiv").data("editor").getValue() },
            success: DefinitionEditor.BindNode,
            complete: function () { kendo.ui.progress($("#treeViewContainer"), false); }
        });
    },

    SetXml: function (xml) {

        $("#xmlViewerDiv").data("editor").setValue(xml);
    },
    getToolbar: function () {
    ...
    StartEditXml: function () {
        $("#xmlViewerDiv").data("editor").setOption("readOnly", false);
        ...
        $("#verticalRootSplitter").data("kendoSplitter").collapse(".k-pane:first");
    },
    ...
    EndEditXml: function () {
        ...
        DefinitionEditor.InitTree(function () {

            var toolbar = DefinitionEditor.getToolbar();

            $("#xmlViewerDiv").data("editor").setOption("readOnly", true);
            ...
            $("#verticalRootSplitter").data("kendoSplitter").expand(".k-pane:first");
        ...
        $("#treeView").kendoTreeView({
                ...
                transport: {
                    read: function (options) {
                        ...
                        $.ajax({
                            ...
                            cache: false,
                            type: "POST",
                            data: { path: options.data.Id, xml: $("#xmlViewerDiv").data("editor").getValue() },
            ...
            select: function (e) {
                ...
                $.ajax({
                    ...
                    cache: false,
                    type: "POST",
                    data: { path: dataItem.Id, xml: $("#xmlViewerDiv").data("editor").getValue() },
                    success: function (result) {
    ...
    ShowOrHideFieldProps: function () {
        ...
        if (inDefCheckbox.length == 0)
            return;

        if (inDefCheckbox[0].checked)
            dvFieldProps.show();
        else
            dvFieldProps.hide();
    },
