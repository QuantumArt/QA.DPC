function addEditDefinitionButton(contentFieldId, xmlFieldId, dpcUrl) {
	...
	QP_CURRENT_CONTEXT.addCustomLinkButton({
		...
		onClick: function (evt) {
			...
			var contentId = parseInt(evt.data.$form.find("[name=field_" + contentFieldId + "]").val());
			...
			var editorWindow = $.telerik.window.create({
				...
				draggable: true,
				scrollable: false,
				onClose: function () { $(this).data('tWindow').content(" "); }
			}).data("tWindow");
			...
			pmrpc.register({
						...
						procedure: function (setXmlProcName) {
							...
							pmrpc.call({
								...
								params: [xml],
								onError: function (statusObj) { console.log("Error calling " + setXmlProcName, statusObj); }
							});
						}
					}
			);
