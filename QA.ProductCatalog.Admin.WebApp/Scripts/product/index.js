...
QA.Product.Index = QA.Product.Index || (function () {
    var ls = window.localStorage;
    var init = function () {
        Quantumart.QP8.Interaction.checkHost(QA.Utils.hostId(), QA.Utils.getParent(), function (args) {
        ...
        $(".actionlink.active").click(function () {
            ...
            QA.Integration.showQPForm($this.data("entityid"), $this.data("parentid"),
                function (eventType, args) {
                    console.log(args);
                    if (args) {
                        if (args.actionCode == "update_article" || args.actionCode == "archive_article" || args.actionCode == "remove_article" ||
                            args.actionCode == "update_article_and_up" || args.actionCode == "save_article_and_up" || args.actionCode == "copy_article") {
                            QA.Utils.refresh();
                        }
                    }
                },
        ...
        $(".anchor").click(function () {
            ...
            if (href) {
                $(href).parents('.collapsed').removeClass("collapsed");
                $.scrollTo($(href), 500);
                return false;
            }
        });
        ...
        $(".toggle-all").click(function () {
            var numCollapsed = 0;
            var items = $(this).closest('.collapsible')
