  ...
  transliterate: function(input) {
    ...
    for (var i = 0; i < input.length; i++) {
      ...
      if (symbolToReplace != undefined) {
        ...
        if (
          currentSymbolCode == Transliterator.dashCode ||
          (currentSymbolCode >= Transliterator.enLettersStartCode &&
            currentSymbolCode <= Transliterator.enLettersEndCode) ||
          (currentSymbolCode >= Transliterator.digitsStartCode &&
            currentSymbolCode <= Transliterator.digitsEndCode)
        )
    ...
    if (result.length > 0 && result[0] == "-") result = result.substr(1);
