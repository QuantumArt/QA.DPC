...
pmrpc = self.pmrpc = (function() {
  ...
  if (
    typeof this.postMessage === "undefined" && // window or worker
    typeof this.onconnect === "undefined"
  ) {
    // shared worker
  ...
  function register(config) {
    if (config.publicProcedureName in reservedProcedureNames) {
      ...
      registeredServices[config.publicProcedureName] = {
        ...
        isAsync:
          typeof config.isAsynchronous !== "undefined"
            ? config.isAsynchronous
            : false,
        acl:
          typeof config.acl !== "undefined"
            ? config.acl
            : { whitelist: ["(.*)"], blacklist: [] }
      };
      return true;
    ...
    if (typeof service !== "undefined") {
      ...
      if (
        ...
        try {
          ...
          return createJSONRpcResponseObject(
            createJSONRpcErrorObject(-1, "Application error.", error.message),
            null,
            id
          );
        }
  ...
  function call(config) {
    ...
    var destContexts = [];
    if (
      typeof config.destination === "undefined" ||
    ...
    for (var i = 0; i < destContexts.length; i++) {
      var callObj = {
        ...
        destinationDomain:
          typeof config.destinationDomain === "undefined"
            ? ["*"]
        ...
        timeout: typeof config.timeout !== "undefined" ? config.timeout : 1500,
  ...
  function discover(params) {
    ...
    var originRegex =
      typeof params.originRegex === "undefined" ? "(.*)" : params.originRegex;
    var nameRegex =
      typeof params.nameRegex === "undefined" ? "(.*)" : params.nameRegex;
    var counter = windowsForDiscovery.length;
    var discoveredMethods = [];
    function addToDiscoveredMethods(methods, destination) {
      for (var i = 0; i < methods.length; i++) {
        if (
          methods[i].origin.match(new RegExp(originRegex)) &&
