...
(function(mod) {
  ...
  var ie_upto7 =
    ie_upto10 && (document.documentMode == null || document.documentMode < 8);
  var ie_upto8 =
    ie_upto10 && (document.documentMode == null || document.documentMode < 9);
  var ie_upto9 =
    ie_upto10 && (document.documentMode == null || document.documentMode < 10);
  ...
  function updateDisplay(cm, viewPort, forced) {
    ...
    for (var first = true; ; first = false) {
      ...
      if (viewPort && viewPort.top != null)
        viewPort = {
          top: Math.min(
            barMeasure.docHeight - scrollerCutOff - barMeasure.clientHeight,
            viewPort.top
          )
        };
      ...
      if (
        visible.from >= cm.display.viewFrom &&
        visible.to <= cm.display.viewTo
      )
        break;
    }
  ...
  function updateDisplayInner(cm, visible, forced) {
    ...
    if (
      !forced &&
      visible.from >= display.viewFrom &&
      visible.to <= display.viewTo &&
      countDirtyView(cm) == 0
    )
      return;
  ...
  function setDocumentHeight(cm, measure) {
    ...
    cm.display.gutters.style.height =
      Math.max(measure.docHeight, measure.clientHeight - scrollerCutOff) + "px";
  }
    ...
    function extendTo(pos) {
      ...
      if (type == "rect") {
        ...
        var anchor = oldRange.anchor,
          head = pos;
        if (type != "single") {
          if (type == "double") var range = findWordAt(cm, pos);
          else
            var range = new Range(
              Pos(pos.line, 0),
    ...
    goLineStartSmart: function(cm) {
      cm.extendSelectionsBy(function(range) {
        ...
        if (!order || order[0].level == 0) {
          ...
          var inWS =
            range.head.line == start.line &&
            range.head.ch <= firstNonWS &&
            range.head.ch;
  ...
  function conflictingCollapsedRange(doc, lineNo, from, to, marker) {
    ...
    if (sps)
      for (var i = 0; i < sps.length; ++i) {
        ...
        if (
          (fromCmp <= 0 &&
            (cmp(found.to, from) || extraRight(sp.marker) - extraLeft(marker)) >
              0) ||
          (fromCmp >= 0 &&
            (cmp(found.from, to) || extraLeft(sp.marker) - extraRight(marker)) <
              0)
        )
          return true;
      }
  ...
  function zeroWidthElement(measure) {
    if (zwspSupported == null) {
      ...
      if (measure.firstChild.offsetHeight != 0)
        zwspSupported =
          test.offsetWidth <= 1 && test.offsetHeight > 2 && !ie_upto7;
    }
