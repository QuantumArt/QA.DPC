...
var jqCronDefaultSettings = {
  ...
  bind_method: {
    set: function($element, value) {
      $element.is(":input")
        ? $element.val(value)
        : $element.data("jqCronValue", value);
    },
    get: function($element) {
      return $element.is(":input")
        ? $element.val()
        : $element.data("jqCronValue");
    }
  }
};
  ...
  var extend = function(dst, src) {
    for (var i in src) {
      if ($.isPlainObject(src[i])) {
        dst[i] = extend(
          dst[i] && $.isPlainObject(dst[i]) ? dst[i] : {},
          src[i]
        );
    ...
    return this.each(function() {
        ...
        console &&
          console.error(
            'Missing translations for language "' +
              settings.lang +
              '". ' +
              "Please include jqCron." +
              settings.lang +
              ".js or manually provide " +
      ...
      if (!settings.jquery_container) {
        if ($this.is(":container")) {
          ...
          // generate new
          settings.jquery_element = $('<span class="jqCron"></span>')
            .uniqueId("jqCron")
            .insertAfter($this);
        } else {
          console &&
            console.error(
              settings.texts[settings.lang].error1.replace("%s", this.tagName)
            );
          return;
        }
      }
  ...
  function jqCron(settings) {
    ...
    this.init = function() {
      ...
      _$obj.bind("cron:change", function(evt, value) {
        ...
        settings.bind_method.set &&
          settings.bind_method.set(settings.bind_to, value);
        _self.clearError();
      });
      ...
      settings.enabled_minute &&
        _selectorPeriod.add("minute", _self.getText("name_minute"));
      settings.enabled_hour &&
        _selectorPeriod.add("hour", _self.getText("name_hour"));
      settings.enabled_day &&
        _selectorPeriod.add("day", _self.getText("name_day"));
      settings.enabled_week &&
        _selectorPeriod.add("week", _self.getText("name_week"));
      settings.enabled_month &&
        _selectorPeriod.add("month", _self.getText("name_month"));
      settings.enabled_year &&
        _selectorPeriod.add("year", _self.getText("name_year"));
      ...
      _selectorMins = newSelector(
        _$blockMINS,
        settings.multiple_mins,
        "minutes"
      );
      ...
      _selectorTimeH = newSelector(
        _$blockTIME,
        settings.multiple_time_hours,
        "time_hours"
      );
      for (i = 0, list = settings.hours; i < list.length; i++) {
        _selectorTimeH.add(list[i], list[i]);
      }
      _selectorTimeM = newSelector(
        _$blockTIME,
        settings.multiple_time_minutes,
        "time_minutes"
      );
      for (i = 0, list = settings.minutes; i < list.length; i++) {
      ...
      _selectorDow = newSelector(
        _$blockDOW,
        settings.multiple_dow,
        "day_of_week"
      );
      ...
      _selectorDom = newSelector(
        _$blockDOM,
        settings.multiple_dom,
        "day_of_month"
      );
      ...
      _selectorMonth = newSelector(
        _$blockMONTH,
        settings.multiple_month,
        "month"
      );
