...
namespace QA.Core.DPC.API
{
    public class ProductAPIService : IProductAPIService
    {
        ...
        public int? CreateProduct(string slug, string version, Article product, bool isLive = false, bool createVersions = false)
        {
            ...
            var insertedList = _productUpdateService.Create(product, productDefinition, isLive, createVersions);

            return insertedList
                .FirstOrDefault(inserted => inserted.ContentId == definition.Content.ContentId)
                ?.CreatedArticleId;
        }
