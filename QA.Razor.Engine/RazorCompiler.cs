...
namespace QA.Razor.Engine
{
    ...
    internal class RazorCompiler
    {
        ...
        private CompilerResults Compile(string className, string template, Encoding encoding, Type modelType = null)
        {
            ...
            using (var reader = new StreamReader(new MemoryStream((encoding ?? Encoding.UTF8).GetBytes(template))))
            {
        ...
        public ITemplate CreateTemplate(string template, Encoding encoding, Type modelType = null)
        {
            string className = Regex.Replace(Guid.NewGuid().ToString("N"), @"[^A-Za-z]*", "");

            var result = Compile(className, template, encoding, modelType);
